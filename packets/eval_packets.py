...
class Schedule:
    ...
    def load_times(self, scenario):
        self.periods = np.genfromtxt(f"./{scenario}/period.csv", delimiter = ",")
        self.times = np.genfromtxt(f"./{scenario}/exec.csv", delimiter = ",")
        self.acc = np.genfromtxt(f"./{scenario}/acc.csv", delimiter = ",")
...
def eval_schedule(schedule):
    ...
    f = open(f"eval_packets_{schedule.schedule}.csv", "x")
